...
class Controller:
    def __init__(self, file_bd='DataBase.db', lc=False, lf=False, file_log='controller.log', test_logging=False,
                 name_test='0'):
    ...
    def modify_user(self, user_id, new_user_info, by_who_id=-1):
        ...
        log = 'User with id {} was modified by {}: '.format(user_id, by_who) + ', '.join(
    ...
    def check_out_doc(self, user_id, doc_id, type_bd='book', returning_time=0):
        ...
        if free_count > 0:
            ...
            if doc_id in current_docs_id:
                self.log('INFO', 'User {} already have copy of document \'{}\''.format(self.get_user(user_id)['name'],
            ...
            self.BDmanager.edit_label("patrons", ["history", "current_books"], [str(history), str(current_orders)],
                                      user_id)
            self.log('INFO', 'User {}({}) want to check out document \'{}\' for {} weeks. Returning time is {}'.format(
                self.get_user(user_id)['name'], self.get_user(user_id)['status'],
                self.get_document(doc_id, type_bd)['title'], returning_time, out_of_time))
            return True, 'OK'
    ...
    def user_get_doc(self, order_id):
        ...
        self.log('INFO', 'User {} get document {}.'.format(self.get_user(order[3])['name'],
                                                           self.get_document(order[3], order[2])['title']))
    ...
    def return_doc(self, user_id, doc_id):
        ...
        self.log('INFO', 'User {} is returned document {}.'.format(self.get_user(user_id)['name'],
    ...
    def modify_document(self, doc, type, by_who_id=0):
        ...
        by_who = 'UNKNOW' if by_who_id == 0 else self.get_user(by_who_id)['name']
        log = 'Document with id {} was modified by {}: '.format(doc_id, by_who) + ', '.join(
            ['new ' + str(key) + ' is ' + str(doc[key]) for key in doc.keys()])
        self.log('INFO', log)
